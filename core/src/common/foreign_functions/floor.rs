...
/// Floor foreign function
///
/// ``` scl
/// extern fn $floor<T: Number>(x: T) -> T
/// ```
#[derive(Clone)]
pub struct Floor;

impl ForeignFunction for Floor {
  fn name(&self) -> String {
    "floor".to_string()
  }
  ...
  fn execute(&self, args: Vec<Value>) -> Option<Value> {
    match args[0] {
      ...
      // Floating points, take floor
      Value::F32(f) => Some(Value::F32(f.floor())),
      Value::F64(f) => Some(Value::F64(f.floor())),
