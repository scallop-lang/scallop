...
pub trait NodeVisitorMut {
  ...
  node_visitor_mut_func_def!(visit_type_decl, TypeDecl);
  ...
  node_visitor_mut_func_def!(visit_algebraic_data_type_decl, AlgebraicDataTypeDecl);
  node_visitor_mut_func_def!(visit_algebraic_data_type_variant, AlgebraicDataTypeVariant);
  node_visitor_mut_func_def!(visit_const_decl, ConstDecl);
  node_visitor_mut_func_def!(visit_const_assignment, ConstAssignment);
  node_visitor_mut_func_def!(visit_entity, Entity);
  node_visitor_mut_func_def!(visit_object, Object);
  ...
  node_visitor_mut_func_def!(visit_rule_head, RuleHead);
  ...
  node_visitor_mut_func_def!(visit_case, Case);
  ...
  node_visitor_mut_func_def!(visit_new_expr, NewExpr);
  node_visitor_mut_func_def!(visit_constant, Constant);
  node_visitor_mut_func_def!(visit_constant_char, ConstantChar);
  node_visitor_mut_func_def!(visit_constant_string, ConstantString);
  node_visitor_mut_func_def!(visit_constant_symbol, ConstantSymbol);
  node_visitor_mut_func_def!(visit_constant_duration, ConstantDuration);
  node_visitor_mut_func_def!(visit_constant_datetime, ConstantDateTime);
  ...
  node_visitor_mut_func_def!(visit_attribute_value, AttributeValue);
  ...
  fn walk_algebraic_data_type_decl(&mut self, algebraic_data_type_decl: &mut AlgebraicDataTypeDecl) {
    self.visit_algebraic_data_type_decl(algebraic_data_type_decl);
    self.visit_location(&mut algebraic_data_type_decl.loc);
    self.walk_identifier(&mut algebraic_data_type_decl.node.name);
    for variant in algebraic_data_type_decl.iter_variants_mut() {
      self.walk_algebraic_data_type_variant(variant);
    }
  }

  fn walk_algebraic_data_type_variant(&mut self, variant: &mut AlgebraicDataTypeVariant) {
    self.visit_algebraic_data_type_variant(variant);
    self.visit_location(&mut variant.loc);
    self.walk_identifier(&mut variant.node.constructor);
    for arg_type in variant.iter_arg_types_mut() {
  ...
  fn walk_const_assignment(&mut self, const_assign: &mut ConstAssignment) {
    ...
    self.walk_entity(const_assign.value_mut())
  }

  fn walk_entity(&mut self, entity: &mut Entity) {
    self.visit_entity(entity);
    self.visit_location(entity.location_mut());
    match &mut entity.node {
      EntityNode::Expr(e) => self.walk_expr(e),
      EntityNode::Object(o) => self.walk_object(o),
    }
  }

  fn walk_object(&mut self, object: &mut Object) {
    self.visit_object(object);
    self.visit_location(object.location_mut());
    self.walk_identifier(object.functor_mut());
    for arg in object.iter_args_mut() {
  ...
  fn walk_rule(&mut self, rule: &mut Rule) {
    ...
    self.walk_rule_head(&mut rule.node.head);
    self.walk_formula(&mut rule.node.body);
  }

  fn walk_rule_head(&mut self, rule_head: &mut RuleHead) {
    self.visit_rule_head(rule_head);
    self.visit_location(rule_head.location_mut());
    match &mut rule_head.node {
      RuleHeadNode::Atom(a) => self.walk_atom(a),
      RuleHeadNode::Conjunction(c) => c.iter_mut().for_each(|a| self.walk_atom(a)),
      RuleHeadNode::Disjunction(d) => d.iter_mut().for_each(|a| self.walk_atom(a)),
    }
  }
  ...
  fn walk_case(&mut self, case: &mut Case) {
    self.visit_case(case);
    self.visit_location(&mut case.loc);
    self.walk_variable(&mut case.node.variable);
    self.walk_entity(&mut case.node.entity);
  }
  ...
  fn walk_atom(&mut self, atom: &mut Atom) {
    ...
    self.visit_location(atom.location_mut());
    self.walk_identifier(atom.predicate_identifier_mut());
    for type_arg in atom.iter_type_arguments_mut() {
      self.walk_type(type_arg);
    }
    for arg in atom.iter_arguments_mut() {
      self.walk_expr(arg);
    }
  }
  ...
  fn walk_new_expr(&mut self, n: &mut NewExpr) {
    self.visit_new_expr(n);
    self.visit_location(&mut n.loc);
    self.walk_identifier(n.functor_identifier_mut());
    for arg in n.iter_args_mut() {
      self.walk_expr(arg);
    }
  }
  ...
  fn walk_constant_char(&mut self, constant_char: &mut ConstantChar) {
    self.visit_constant_char(constant_char);
    self.visit_location(constant_char.location_mut());
  }

  fn walk_constant_string(&mut self, constant_string: &mut ConstantString) {
    self.visit_constant_string(constant_string);
    self.visit_location(constant_string.location_mut());
  }

  fn walk_constant_symbol(&mut self, constant_symbol: &mut ConstantSymbol) {
    self.visit_constant_symbol(constant_symbol);
    self.visit_location(constant_symbol.location_mut());
  }

  fn walk_constant_datetime(&mut self, constant_datetime: &mut ConstantDateTime) {
    self.visit_constant_datetime(constant_datetime);
    self.visit_location(constant_datetime.location_mut());
  }

  fn walk_constant_duration(&mut self, constant_duration: &mut ConstantDuration) {
    self.visit_constant_duration(constant_duration);
    self.visit_location(constant_duration.location_mut());
  }
  ...
  fn walk_attribute_value(&mut self, attribute_value: &mut AttributeValue) {
    self.visit_attribute_value(attribute_value);
    self.visit_location(&mut attribute_value.loc);
    match &mut attribute_value.node {
...
macro_rules! impl_node_visitor_mut_tuple {
  ( $($id:ident,)* ) => {
      ...
      $($id: NodeVisitorMut,)*
    {
      ...
      node_visitor_mut_visit_node!(visit_type_decl, TypeDecl, ($($id),*));
      ...
      node_visitor_mut_visit_node!(visit_algebraic_data_type_decl, AlgebraicDataTypeDecl, ($($id),*));
      node_visitor_mut_visit_node!(visit_algebraic_data_type_variant, AlgebraicDataTypeVariant, ($($id),*));
      ...
      node_visitor_mut_visit_node!(visit_entity, Entity, ($($id),*));
      node_visitor_mut_visit_node!(visit_object, Object, ($($id),*));
      ...
      node_visitor_mut_visit_node!(visit_rule_head, RuleHead, ($($id),*));
      ...
      node_visitor_mut_visit_node!(visit_case, Case, ($($id),*));
      ...
      node_visitor_mut_visit_node!(visit_call_expr, CallExpr, ($($id),*));
      node_visitor_mut_visit_node!(visit_new_expr, NewExpr, ($($id),*));
      node_visitor_mut_visit_node!(visit_constant, Constant, ($($id),*));
      node_visitor_mut_visit_node!(visit_constant_char, ConstantChar, ($($id),*));
      node_visitor_mut_visit_node!(visit_constant_string, ConstantString, ($($id),*));
      node_visitor_mut_visit_node!(visit_constant_symbol, ConstantSymbol, ($($id),*));
      node_visitor_mut_visit_node!(visit_constant_datetime, ConstantDateTime, ($($id),*));
      node_visitor_mut_visit_node!(visit_constant_duration, ConstantDuration, ($($id),*));
      ...
      node_visitor_mut_visit_node!(visit_attribute_value, AttributeValue, ($($id),*));
    }
  }
}
