...
fn test_static_project_1() {
  ...
  // Relations
  let mut source = StaticRelation::<(i8, i8), unit::Unit>::new();
  let mut target = StaticRelation::<(i8, i8), unit::Unit>::new();
  ...
  while source.changed(&ctx) || target.changed(&ctx) {
    target.insert_dataflow_recent(&ctx, project(&source, |t| (t.0, t.1 + 1)))
  }
