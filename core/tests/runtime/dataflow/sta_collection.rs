...
fn test_static_collection_dataflow_1() {
  ...
  let col = StaticCollection::<(i8, i8), unit::Unit>::from_vec(
    vec![
  ...
  let mut other = StaticRelation::<(i8, i8), unit::Unit>::new();
  let mut first_time = true;
  while other.changed(&ctx) || first_time {
    other.insert_dataflow_recent(&ctx, dataflow::collection(&col, first_time));
    first_time = false;
  }
