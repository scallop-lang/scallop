  ...
  T::Context: ProvenanceContext + Default,
{
  ...
  // Insert updates
  iter.add_update_dataflow("path", Dataflow::dynamic_relation("edge"));
  iter.add_update_dataflow(
    "_edge_rev",
    Dataflow::dynamic_relation("edge").project((Expr::access(1), Expr::access(0))),
  );
  iter.add_update_dataflow(
    "path",
    Dataflow::dynamic_relation("_edge_rev")
      .join(Dataflow::dynamic_relation("path"))
