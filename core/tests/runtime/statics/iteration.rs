...
fn test_static_iter_edge_path() {
  ...
  while iter.changed() || iter.is_first_iteration() {
    ...
    iter.insert_dataflow(
      &path,
      dataflow::project(iter.join(&path_inv, &edge), |t| (t.1, t.2)),
    );
    iter.step();
  }
...
fn test_static_iter_odd_even_3() {
  ...
  fn stratum_0<C: ProvenanceContext>(prov: &mut C) -> Stratum0Result<C> {
    ...
    while iter.changed() || iter.is_first_iteration() {
      ...
      iter.insert_dataflow(
        &_numbers_perm_0_0,
        dataflow::project(&numbers, |t| (t.0, t.0)),
      );
      iter.insert_dataflow(
        &_numbers_perm_0_,
        dataflow::project(&numbers, |t| (t.0, ())),
      );
      iter.step();
    }
    Stratum0Result {
  ...
  fn stratum_1<C: ProvenanceContext>(
    prov: &mut C,
    ...
    while iter.changed() || iter.is_first_iteration() {
      iter.insert_dataflow(
        ...
        dataflow::project(
          dataflow::collection(
            &stratum_0_result._numbers_perm_0_0,
            iter.is_first_iteration(),
          ),
          |t| (t.0 - 2, t.0),
        ),
      );
      iter.insert_dataflow(
        ...
        dataflow::project(
          dataflow::project(
            dataflow::find(
              dataflow::collection(
                &stratum_0_result._numbers_perm_0_,
                iter.is_first_iteration(),
              ),
              1,
            ),
      ...
      iter.insert_dataflow(
        &odd,
        dataflow::project(
          dataflow::project(iter.join(&_temp_0, &_odd_perm_0), |t| t.1),
          |t| (t,),
        ),
      );
  ...
  expect_static_collection(
    &stratum_2_result.even,
    vec![(0,), (2,), (4,), (6,), (8,), (10,)],
  );
}
