...
class MNISTAddMod3Net(nn.Module):
  def __init__(self, provenance, k):
    ...
    self.scl_ctx.add_rule("add_mod_3((a + b) % 3) :- digit_1(a), digit_2(b)")

    # The `add_mod_3` logical reasoning module
    self.add_mod_3 = self.scl_ctx.forward_function("add_mod_3", output_mapping=list(range(3)))
  ...
  def forward(self, x: Tuple[torch.Tensor, torch.Tensor]):
    ...
    return self.add_mod_3(digit_1=a_distrs, digit_2=b_distrs) # Tensor 64 x 3
...
class Trainer():
  def __init__(self, train_loader, test_loader, learning_rate, loss, k, provenance):
    self.network = MNISTAddMod3Net(provenance, k)
...
if __name__ == "__main__":
  ...
  # Dataloaders
  train_loader, test_loader = mnist_add_mod_3_loader(data_dir, batch_size_train, batch_size_test)
