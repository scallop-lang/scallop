...
class MNISTSumDouble2Dataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, idx):
    ...
    return (a_img, b_img, c_img, d_img, a_digit * 10 + b_digit + c_digit * 10 + d_digit)
...
class MNISTSumDouble2Net(nn.Module):
  def __init__(self, provenance, k):
    ...
    self.scl_ctx.add_rule("sum_double_2(a * 10 + b + c * 10 + d) :- digit_1(a), digit_2(b), digit_3(c), digit_4(d)")
