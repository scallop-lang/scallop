...
class MNISTSum4Dataset(torch.utils.data.Dataset):
  ...
  def __len__(self):
    return int(len(self.mnist_dataset) / 4)
...
class MNISTSum4Net(nn.Module):
  def __init__(self, provenance, k):
    ...
    self.scl_ctx.add_rule("sum_4(a + b + c + d) = digit_1(a), digit_2(b), digit_3(c), digit_4(d)")

    # The `sum_4` logical reasoning module
    self.sum_4 = self.scl_ctx.forward_function("sum_4", [(i,) for i in range(37)])
  ...
  def forward(self, x: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]):
    ...
    return self.sum_4(digit_1=a_distrs, digit_2=b_distrs, digit_3=c_distrs, digit_4=d_distrs)
...
class Trainer():
  def __init__(self, train_loader, test_loader, learning_rate, loss, k, provenance):
    self.network = MNISTSum4Net(provenance, k)
...
if __name__ == "__main__":
  # Argument parser
  parser = ArgumentParser("mnist_sum_4")
  ...
  # Dataloaders
  train_loader, test_loader = mnist_sum_4_loader(data_dir, batch_size_train, batch_size_test)
