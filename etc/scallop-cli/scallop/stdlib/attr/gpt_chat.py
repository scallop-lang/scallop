  ...
  *,
  ...
  def invoke_gpt(*args):
    ...
    # Need to do a new request
      ...
      _, current_conversation  = query_gpt_completion(system_ctx, messages, openai_config.MODEL)
    ...
    for response in responses:
      tup = parse_choice_text(response.strip(), arg_types, num_bounded)
      yield tup
...
def fill_template(methodology_sub_texts):
  template_text = ''' Answer in the format of new-line separated \"BLANK_VAR = ANSWER\".
    Here is one example:
  ...
  messages = [{"role": "user", "content": template_text + text} for text in methodology_sub_texts]
