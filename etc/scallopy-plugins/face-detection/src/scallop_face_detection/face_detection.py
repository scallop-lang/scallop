...
def get_face_model():
    ...
    if _FACE_MODEL is None:
        try:
            _FACE_MODEL = face_detection_lib.build_detector(
                "DSFDDetector",
...
def face_detection(
    ...
    *,
    ...
    ), f"{ERR_HEAD} The parameter `output_fields` need to be a list"
    assert (
        len(output_fields) > 0
    ), f"{ERR_HEAD} The `output_fields` list must be non-empty"
    for field in output_fields:
        assert (
            field in FIELDS
        ), f"{ERR_HEAD} Unknown output field `{field}`; supported fields are {list(FIELDS.keys())}"
    expected_args += [
        (FIELDS[f]["type"], f + ": " + FIELDS[f]["type"]) for f in output_fields
    ]

    # Check arity of relation
    assert len(arg_types) == len(
        expected_args
    ), f"{ERR_HEAD} relation has to be of arity-{len(expected_args)}: {[s for (_, s) in expected_args]}"

    # Making sure that arg types are as expected
    for arg_id in range(len(arg_types)):
        (expected_ty, s), arg_name, arg_ty = (
            expected_args[arg_id],
            arg_names[arg_id],
            arg_types[arg_id].name(),
        )
        if arg_ty.lower() != expected_ty.lower():
            if arg_name is not None:
                raise Exception(
                    f"{ERR_HEAD} Type mismatch for `{arg_name}`. Expected `{s}`, but found arg of type `{arg_ty}`"
                )
            else:
                raise Exception(
                    f"{ERR_HEAD} Type mismatch for argument `{arg_id+1}`. Expected `{s}`, but found arg of type `{arg_ty}`"
                )
    ...
    def do_face_detect(img: scallopy.Tensor):
        ...
        for i, box in enumerate(faces):
            ...
            w, h = int((enlarge_face_factor - 1) * (x2 - x1) / 2), int(
                (enlarge_face_factor - 1) * (y2 - y1) / 2
            )
            x1, y1, x2, y2 = (
                max(0, x1 - w),
                max(0, y1 - h),
