...
def clip(
    ...
    *,
    ...
    assert (
        item.is_relation_decl()
    ), f"{ERR_HEAD} has to be an attribute of a relation type declaration"
    assert (
        len(item.relation_decls()) == 1
    ), f"{ERR_HEAD} cannot be an attribute on multiple relations"
    ...
    assert (
        len(args) >= 1 and args[0].ty.is_tensor() and args[0].adornment.is_bound()
    ), f"{ERR_HEAD} first argument has to be a bounded type `Tensor`"
    if labels is not None:
        assert (
            len(args) == 2
        ), f"{ERR_HEAD} relation has to be of arity-2 provided the labels"
        assert args[1].ty.is_string() and (
            args[1].adornment is None or args[1].adornment.is_free()
        ), f"{ERR_HEAD} second argument has to be of free type `String`"
    else:
        assert (
            len(args) == 3
        ), f"{ERR_HEAD} relation has to be of arity-3 given that labels need to be passed in dynamically"
        assert (
            args[1].ty.is_string() and args[1].adornment.is_bound()
        ), f"{ERR_HEAD} second argument has to be a bounded type `String`"
        assert args[2].ty.is_string() and (
            args[2].adornment is None or args[2].adornment.is_free()
        ), f"{ERR_HEAD} third argument has to be of free type `String`"
    ...
    def clip_classify(img: scallopy.Tensor) -> scallopy.Facts[float, Tuple[str]]:
        ...
        with torch.no_grad():
            class_prompts = [
                get_class_prompt(prompt, class_str) for class_str in labels
            ]
            txt_tokens = clip_lib.tokenize(class_prompts).to(device=device)
            img_proc = (
                clip_preprocess(Image.fromarray(img.numpy()))
                .unsqueeze(0)
                .to(device=device)
            )
            ...
            for prob, class_prompt, class_str in zip(probs, class_prompts, labels):
                if debug:
                    print(
                        f"[@clip_classifier] {prob:.4f} :: {class_prompt} (Class Name: {class_str})"
                    )
    ...
    @scallopy.foreign_predicate(name=relation_decl.name.name)
    def clip_classify_with_labels(
        img: scallopy.Tensor, list: scallopy.String
    ) -> scallopy.Facts[float, Tuple[str]]:
        nonlocal labels
