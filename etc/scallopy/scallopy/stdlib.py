...
def soft_eq_tensor(x: value_types.Tensor, y: value_types.Tensor) -> Facts[value_types.Tensor, Tuple]:
  import torch
...
def soft_neq_tensor(x: value_types.Tensor, y: value_types.Tensor) -> Facts[value_types.Tensor, Tuple]:
  import torch
  cs = torch.cosine_similarity(x, y, dim=0)
  prob = 1.0 - (cs + 1.0 / 2.0)
  yield (prob, ())
